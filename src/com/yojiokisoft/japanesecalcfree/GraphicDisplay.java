...
public class GraphicDisplay extends AbstractDisplay {
	...
	public GraphicDisplay(ViewGroup viewGroup, Context context, int orientation) {
		...
		Pair<Integer, Integer> wh = MyResource
		...
		if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
			...
			mCharHeight = (displaySize - mMemoryWidth - margin)
					/ (DISPLAY_DIGIT + 1);
		} else {
			displaySize = wh.first;
			mCharWidth = (displaySize - mMemoryWidth - margin)
					/ (DISPLAY_DIGIT + 1);
		...
		for (int i = DISPLAY_DIGIT; i >= 0; i--) {
			...
			linearLayout.addView(mNum[i], new LinearLayout.LayoutParams(
		...
		for (int i = sb.length() - 1; i >= 0; i--) {
			...
			case '.':
				...
				if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
					...
					top = mCharHeight * (sb.length() - 1 - index)
							+ mMemoryWidth - marginX;
				} else {
					left = mCharWidth * (DISPLAY_DIGIT - index + 1)
							+ mMemoryWidth - marginX;
		...
		for (int i = 0; i < keta.length; i++) {
			if (ketaSu > keta[i]) {
				...
				if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
					left = mCharWidth - margin3;
					top = mCharHeight
							* (sb.length() - keta[i] - (mDecimalPlaces == 0 ? 0
									: mDecimalPlaces + 1)) + mMemoryWidth
							- marginX;
				} else {
					left = mCharWidth
							* (DISPLAY_DIGIT - keta[i] - mDecimalPlaces + 1)
							+ mMemoryWidth - marginX;
					top = mCharHeight - margin1;
				}
	...
	/**
	 * スタックから文字列へ 例)displayChar:123 decimalPlaces:2 commaMode:true minus:true →
	 * -1.23
	 * 
	 * @return 文字列化した数値
	 */
	...
	/**
	 * 文字列からスタックへ 例)-1.23 → displayChar:123 decimalPlaces:2 commaMode:true
	 * minus:true
	 */
